...
var floorSize=80,wallSize=5,player={size:floorSize-5,speed:10,x:0,y:0,velX:0,velY:0},ctx=document.getElementById("game").getContext("2d");ctx.canvas.width=Math.round((window.innerWidth-2*floorSize)/floorSize)*floorSize+wallSize;var WIDTH=ctx.canvas.width-wallSize,HEIGHT=ctx.canvas.height-wallSize,rows=Math.floor(HEIGHT/floorSize),cols=Math.floor(WIDTH/floorSize),map=new Array(rows),exit=null,game=null,animation=null;
